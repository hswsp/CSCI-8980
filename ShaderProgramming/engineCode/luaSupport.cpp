...
void luaSetup(lua_State * L){
	...
	lua_register(L, "resetModelTransform", resetModelTransform);
	lua_register(L, "setModelTransform", setModelTransform);
	...
	lua_register(L, "intersectMouseWithLayer",intersectMouseWithLayer);
...
int playSoundEffect(lua_State * L){ //plays background music
	...
	LOG_F(1,"Playing Sound Effect ID '%d'",audioID); //TODO: Store the name of the sound effect somewhere
...
int addModel(lua_State * L){
	...
	if (!pooledModel){
		...
		string modelName = childModelName + string("-Lua-")+ std::to_string(luaModelCount);
		luaModelCount++;
...
int setModelTransform(lua_State * L){
	...
	//Read 16 table values from Lua
	glm::mat4 t;
	int index = 1; //1-indexed current table value
	for (int i = 0; i < 4; i++){
		for (int j = 0; j < 4; j++){
			lua_rawgeti(L, 2, index++);
			t[j][i] = lua_tonumber(L,-1); //Reads the value from the raw get
		}
	}
	LOG_F(2,"Setting model %s to %f %f %f %f ...",models[modelID].name.c_str(), t[0][0], t[0][1], t[0][2], t[0][3]);

	models[modelID].transform = t;
