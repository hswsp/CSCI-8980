...
void luaSetup(lua_State * L){
	...
	lua_register(L, "resetModelTansform", resetModelTansform);
...
int playSoundEffect(lua_State * L){ //plays background music
	...
	LOG_F(INFO,"Playing Sound Effetct ID '%d'",audioID); //TODO: Store the name of the sound effect somewhere
...
int addCollider(lua_State * L){
	...
	int colliderID = addCollider(modelID,layer, fmaxf(r,models[modelID].boundingRadius),glm::vec3(tx,ty,tz));
