...
int main(int argc, char *argv[]){
	...
	while (!quit){
		...
		if (starttime(L)==1 && start_t==0){
			...
			//only go to this loop once
			// time_t temp = std::time(0);   // get time now
        	// start = std::localtime(&temp);
			// hour=start->tm_hour;
			// minutes=start->tm_min;
			// seconds=start->tm_sec;
			
        //   temp = chrono::system_clock::now();
		//   time_t temp2 = chrono::system_clock::to_time_t(temp);
		//   start =localtime(&temp2);
		}
		...
		if(!lua_isfunction(L,-1)){ //make having frameUpdate function optional
      lua_pop(L, 1);
    }	else{
		...
		if (useShadowMap && curScene.shadowLight.castShadow){
			...
			auto z = lookatPoint-lightPos;
			lightUp = glm::cross(camUp,lookatPoint-lightPos); //TODO: Is this hacky? What should the light up be?
			lightViewMatrix = glm::lookAt(lightPos,lookatPoint, lightUp); 

			if (shadowFrustumCull){
				computeShadowDepthMap(lightViewMatrix, lightProjectionMatrix, curScene.toDraw, curScene.shadowLight.frustLeft, curScene.shadowLight.frustRight, curScene.shadowLight.frustTop, curScene.shadowLight.frustBot, curScene.shadowLight.frustNear, curScene.shadowLight.frustFar);
			} else{
		...
		// int sec=(int(now->tm_hour)-hour)*3600+(int(now->tm_min)-minutes)*60+int(now->tm_sec)-sec;
		// ImGui::Text("Time now:%d:%d:%d", now->tm_hour,now->tm_min,now->tm_sec);
