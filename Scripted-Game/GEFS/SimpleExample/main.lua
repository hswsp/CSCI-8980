Init = require "SimpleExample/Init"
Finish = require "SimpleExample/End"

require "SimpleExample/Animation"
...
function frameUpdate(dt)
  ...
  if GameContrl == 1 then
    ...
    for modelID,v in pairs(Anim) do
      ...
      if time==2 and InitialEndflag == false then
        CameraPosX = 0.0
        CameraPosY = 2.5
        CameraPosZ = -7.0
...
function mouseHandler(mouse)
    ...
    elseif(mouse.left and mouse_released) then -- make sure the first time clicked
      ...
      if(GameContrl == 1) then
        -- the object layer
        local hitmodelID, hitmodelDIst = getMouseClickWithLayer(1)   
        -- the board layer
        local hittileID, hittileDIst = getMouseClickWithLayer(0)
        local curMID = nil
    ...
    if(mouse.right) then
      hitmodelID, hitmodelDIst = getMouseClickWithLayer(1)
      if(hitmodelID~=nil) then
